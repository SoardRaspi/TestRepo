...
def recenter(curr_coords, Harris_COI_coords, angle_next, step_wanted, Harris_directions, DIRECTIONS, MODE):
    ...
    if flag_found == 0:
        if max_ratio_match != 0:
            ...
            X_, Y_ = kernel_operator(Harris_directions[0], Harris_COI_Y, Harris_COI_X, getstep(Harris_directions[0],
                                                                                               Harris_COI_Y, Harris_COI_X, coords_for_COI))
    ...
    while len(directions) != 0:
        ...
        if len(directions) == 1:
            ...
            # if not prev:
                ...
                print("key at len(directions) = 1:", key)
                ...
                if not key:
                    ...
                    if key:
                        ...
                        X, Y = recenter([X, Y], [XX, YY], 0, 1, directions, directions, 1)
                        Y, X = kernel_operator(directions[0], Y, X, 1)
                        n, prob_arr, directions = kernel_determiner(image_final, X, Y, 1.1, 0.901)
...
key = None
_, __, ___, X1, Y1 = grapher_3D(21, image_final, X, Y)
for k in coords_for_COI:
    if coords_for_COI[k] == [X1, Y1]:
        key = k
...
X, Y = recenter([X, Y], [X1, Y1], Harris_corners_angles[key], 1, Harris_corners_directions[key],
                Harris_corners_directions[key], 0)
mainFunction(X, Y, None, 1, None)

_, __, ___, X1, Y1 = grapher_3D(13, image_final, 391, 504)
directions, pp, ____, angle = get_dirs_from_coords(_, __, ___, X1, Y1, None)
